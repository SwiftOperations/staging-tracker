window.openOrderHistory = async function(so) {
  ...
  try {
    ...
    html += `<h4 style="margin:0 0 8px 0; color:#0ea5e9; border-bottom:1px solid #cbd5e1; padding-bottom:4px;">Current Active Staging</h4>`;
    ...
    html += `<h4 style="margin:0 0 8px 0; color:#10b981; border-bottom:1px solid #cbd5e1; padding-bottom:4px;">Past Shipments</h4>`;
    if(shippedEntries.length === 0) html += `<p style="font-size:12px; color:#6b7280;">No past shipments found.</p>`;
      ...
      shippedEntries.forEach(e => {
        ...
        html += `<li><b>${e.type}</b> - ${action} from ${e.location} <br><span style="font-size:11px; color:#64748b;">(By ${e.shipped_by || 'Unknown'} on ${new Date(e.shipped_at).toLocaleString()})</span></li>`;
      });
      html += `</ul>`;
    }
...
window.openChangelogModal = async function(table) {
  ...
  } catch(e) { tbody.innerHTML = `<tr><td colspan="2" style="text-align:center; color:red; padding:12px;">Error: ${e.message}</td></tr>`; }
};