...
window.getDynamicType = function(prefix) {
  ...
  let typeParts = []; 
  if(sk) typeParts.push(window.formatContainer(sk, 'Skid')); if(bx) typeParts.push(window.formatContainer(bx, 'Box'));
  if(cr) typeParts.push(window.formatContainer(cr, 'Crate')); if(pi) typeParts.push(window.formatContainer(pi, 'Pipe/Rod'));
...
window.renderTables = function() {
    ...
    if(sBody) {
      ...
      fStaging.slice(0, limitStaging).forEach(o => {
        ...
        sBody.insertAdjacentHTML('beforeend', `<tr>
          ...
          <td class="hide-in-batch">${picBtn}</td><td><a class="so-link" onclick="event.stopPropagation(); window.openOrderHistory('${o.so}')">${o.so}</a></td><td>${o.customer}</td><td>${new Date(o.entry_date).toLocaleString()}</td><td>${o.type}</td><td>${o.location}</td><td><small>${geoLink}</small></td>
    ...
    if(shBody) {
      ...
      fShipped.slice(0, limitShipped).forEach(o => {
        ...
        shBody.insertAdjacentHTML('beforeend', `<tr ${rowClass}>
          <td>${editBtn}</td>
          <td>${picBtn}</td>
          <td><a class="so-link" onclick="event.stopPropagation(); window.openOrderHistory('${o.so}')">${o.so}</a></td><td>${o.customer}</td><td>${o.type}</td><td>${o.carrier || '—'}</td><td>${o.location}</td><td><small>${geoLink}</small></td>
  ...
  if($('#kSkids')) $('#kSkids').textContent = sumByType('Skid'); if($('#kBoxes')) $('#kBoxes').textContent = sumByType('Box'); 
  if($('#kCrates')) $('#kCrates').textContent = sumByType('Crate'); if($('#kPipe')) $('#kPipe').textContent = sumByType('Pipe/Rod'); 
...
window.openUniversalEditor = function(table, id) {
  ...
  if($('#e_so')) $('#e_so').value = o.so; if($('#e_cust')) $('#e_cust').value = o.customer; 
  if($('#e_loc')) $('#e_loc').value = o.location || ''; if($('#e_coords')) $('#e_coords').value = o.coords || ''; 
  ...
  if($('#e_skid')) $('#e_skid').value = counts.sk; if($('#e_box')) $('#e_box').value = counts.bx; 
  if($('#e_crate')) $('#e_crate').value = counts.cr; if($('#e_pipe')) $('#e_pipe').value = counts.pi; if($('#e_other')) $('#e_other').value = counts.ot;
  ...
  if (table === 'staging') {
    ...
    if($('#e_status')) $('#e_status').value = o.status; if($('#e_staged_by')) $('#e_staged_by').value = o.staged_by || '';
  } else {
  ...
  window.renderEditPhotoStrip(); if($('#editModal')) $('#editModal').style.display = 'flex';
};
...
window.triggerReturnModal = function() {
  ...
  if($('#r_picked_by')) $('#r_picked_by').value = ''; if($('#r_returned_by')) $('#r_returned_by').value = ''; 
  if($('#r_reason')) $('#r_reason').value = ''; if($('#r_pm_chk')) $('#r_pm_chk').checked = false; 
  window.togglePMEmail(false, 'r_pm_email', 'r_pm_email_btn'); if($('#r_pm_email')) $('#r_pm_email').value = ''; 
};
...
window.triggerShipModal = function(id) {
  ...
  activeShipTargetItem = item; if($('#photoPreviewStrip')) $('#photoPreviewStrip').innerHTML = ''; selectedPhotoBlobs = [];
  ...
  if($('#shipModal')) $('#shipModal').style.display = 'flex'; window.renderPhotoStrip('#photoPreviewStrip', selectedPhotoBlobs);
};

window.closeShipModal = function() {
  if($('#shipModal')) $('#shipModal').style.display = 'none'; 
  window.loadCloudData();
  if (window.activeReportMode) window.pauseReport();
};
  ...
  Object.keys(groups).forEach(so => {
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr class="group-header-row" data-so="${so}" data-cust="${allCustomers}" data-safeid="${safeId}" style="cursor:pointer; background:#f1f5f9;" onclick="window.toggleOrderGroup('${safeId}')">
        <td style="padding: 8px 12px;"><span id="icon_so_${safeId}">+</span> <a class="so-link" onclick="event.stopPropagation(); window.openOrderHistory('${so}')">${so}</a></td>
    ...
    groups[so].forEach(o => {
      tbody.insertAdjacentHTML('beforeend', `
        <tr class="sub_so_${safeId}" style="display:none; font-size:12px; background:#fff;">
          <td style="padding: 6px 12px 6px 24px; color:#4b5563;">↳ ${o.customer}</td><td style="padding: 6px 12px; white-space: nowrap;">${o.type}</td>
          <td style="padding: 6px 12px; white-space: nowrap;">${o.location}</td><td style="padding: 6px 12px; color:#6b7280; text-align:right; white-space:nowrap;">${new Date(o.entry_date).toLocaleString()}</td>
        </tr>
      `);
    });
  });
...
window.filterOrdersModal = function() {
  ...
  document.querySelectorAll('.group-header-row').forEach(tr => {
    ...
    if(!match) { document.querySelectorAll('.sub_so_' + safeId).forEach(r => r.style.display = 'none'); const icon = document.getElementById('icon_so_' + safeId); if(icon) icon.textContent = '+'; }
  });
};

window.toggleOrderGroup = function(safeId) {
   const rows = document.querySelectorAll('.sub_so_' + safeId); const icon = document.getElementById('icon_so_' + safeId);
...
window.showNotification = function(message) {
  ...
  const toast = document.createElement('div'); toast.className = 'toast-msg'; toast.textContent = message; container.appendChild(toast);
