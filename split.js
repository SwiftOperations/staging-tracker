...
window.submitSplitCount = function() {
  ...
  window.splitEngine.total = count; window.splitEngine.current = 1; window.splitEngine.dataArray = [];
...
window.renderSplitConfig = function() {
  ...
  $('#sp_so').value = item.so; $('#sp_cust').value = item.customer;
  ...
  $('#sp_status').value = 'Partial'; $('#sp_staged_by').value = currentUser ? currentUser.email.split('@')[0] : '';
